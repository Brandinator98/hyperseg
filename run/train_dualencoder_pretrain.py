...
valid_datasets = ['hsidrive','whuohs','hyko2', 'hsiroad', 'hcv2', 'loveda']
valid_models = ['unet', 'agunet', 'spectr', 'deeplabv3plus', 'resnet_dualencoder']

def make_reproducible(manual_seed=42):
    seed_everything(manual_seed, workers=True)
    torch.use_deterministic_algorithms(True, warn_only=True)

@hydra.main(version_base=None, config_path="conf", config_name="train_conf_pretrain")
def train(cfg):
    ...
    ## Add weights here via reload path
    model = get_model(cfg.model, cfg.training.preload_path)
